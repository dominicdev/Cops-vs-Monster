...
function functions.onSceneTouch (event)
    ...
    if event.phase == "ended" and event.target.id == "restart_survival" then
        local option = 
            {
            effect = "fade",
            time = 500,
    ...
    elseif event.phase == "ended" and event.target.id == "quit_gameover" then
        local stats = 
                {
                    effect = "fade",
                    time   = 300,
                    params = 
                    {
                        uscore  = number_.score,
                        utime   = params.stage,
                        ucar    = number_.carP,
                        ubarrel = number_.laserP,
                        ulevel  = params.level,
                        ulaser  = number_.laserP,
                        ucoin   = number_.coin,
                        gametype= "survival",
...
function functions.winnerscreen (event)
...
function functions.gameoverscreen ()    
    external.physics.stop()
...
function functions.pauseall (event)
    ...
    if event.phase == "ended" or (event.keyName == "back" and event.phase == "down") then
        ...
        if game_.pause == true and game_.over == false and game_.winner == false then
            ...
            if bol.bolsstats == true then
                timer.pause(timer_.bolsstats)
            end
                
            game_.pause = false
            
        elseif game_.pause == false and game_.over == false and game_.winner == false then
...
function functions.bombhit(object,locx,locy)
local x1 = locx
local y1 = locy
--print(object)
    if  object == "runnerers" then
...
bol.laser = true
number_.deadmon = number_.deadmon + 1
dead[number_.deadmon] = external.sprite.newSprite(external.spritefactory.spritelaser)
dead[number_.deadmon].x = locx 
dead[number_.deadmon].y = locy
dead[number_.deadmon]:prepare("strike")  
dead[number_.deadmon]:play()
group[6]:insert(dead[number_.deadmon])
dead[number_.deadmon]:addEventListener( "sprite", functions.spriteListener )
external.physics.addBody(dead[number_.deadmon],"static",{density = 1, bounce = 0,isSensor = true});
dead[number_.deadmon].myname = "beam"
dead[number_.deadmon].alpha = 0.8
bol.laser = false
...
audio.play(external.sfx.sound_3,{channel = 7})
...
function functions.bigmastercall (event)
    ...
    external.physics.addBody(monsters[number_.monster] ,{density = 0, bounce = 0,firction = 0})
    monsters[number_.monster].isFixedRotation = true
    monsters[number_.monster]:applyForce(0, number_.bigspeed, monsters[number_.monster].x , monsters[number_.monster].y)
...
boss[boss.num].y = -10
boss[boss.num]:prepare("shipfront"); 
boss[boss.num]:play()  
...
elseif game_.stage == 40 and game_.level == 2 then
game_.level = 1
game_.stage = 1
game_.leveltemp = game_.leveltemp + 1
game_.stagetemp = 1
else
game_.stage = game_.stage + 1  
...
bol.bols            = row.bossbol
bol.masstatus       = row.masstatus
bol.bigmas          = row.bigmas
bol.monmovstats     = row.monmovstats
number_.masdamage   = row.masdamage
number_.bigdamage   = row.bigdamage
number_.damage      = row.damage
game_.mobspeed      = row.speed
game_.masspeed      = row.masspeed
number_.bigspeed    = row.bigspeed


end

if bol.masstatus == "false" then
objects_.master.alpha = 0
text_.master.alpha = 0
end

if bol.bigmas == "false" then
text_.bigmaster.alpha = 0
objects_.bigmaster.alpha = 0
end

function functions.startnewgame (event)
...
transition.to(text_.count,{alpha = 0,delay = 2000,time = 1000,onComplete = functions.startnewgame})

text_.level:setText("Level "..game_.leveltemp.." ".."Wave-"..game_.stagetemp);
...
if game_.weather_4 == true then
    
function functions.changeback_1 () 
    objects_.dark:removeSelf()
    objects_.dark = nil
    objects_.fog_1:removeSelf()
    objects_.fog_1 = nil
    objects_.fog_2:removeSelf()
    objects_.fog_2= nil
    ...
    transition.to(objects_.dark,{time = 1000,alpha = 1})
end
transition.to(objects_.fog_1,{time = 2000,alpha = 0})
transition.to(objects_.fog_2,{time = 2000,alpha = 0})
transition.to(objects_.dark,{time = 2000,alpha = 0,onComplete = functions.changeback_1})
end
game_.weather_1 = true
game_.weather_2 = false 
game_.weather_3 = false
game_.weather_4 = false
elseif game_.stage >= 6  and game_.stage <= 10 or game_.stage >= 26  and game_.stage <= 30 then

if game_.weather_1 == true then
function functions.changeback_2 () 
    objects_.dark:removeSelf()
    objects_.dark = nil
...
transition.to(objects_.dark,{time = 2000,alpha = 0,onComplete = functions.changeback_2})

end
game_.weather_1 = false
game_.weather_2 = true  
game_.weather_3 = false
game_.weather_4 = false

elseif game_.stage >= 11  and game_.stage <= 15 or game_.stage >= 31  and game_.stage <= 35 then
if game_.weather_2 == true then    
function functions.changeback_3 () 
    objects_.dark:removeSelf()
    objects_.dark = nil
...
transition.to(objects_.dark,{time = 2000,alpha = 0,onComplete = functions.changeback_3})
end
game_.weather_1 = false
game_.weather_2 = false  
game_.weather_3 = true
game_.weather_4 = false

elseif game_.stage >= 16  and game_.stage <= 20 or game_.stage >= 26  and game_.stage <= 40 then
if game_.weather_3 == true then  
function functions.changeback_4 ()    
    ...
    transition.to(objects_.fog_1,{time = 1000,alpha = 1})
    transition.to(objects_.fog_2,{time = 1000,alpha = 1})
    transition.to(objects_.dark,{time = 1000,alpha = 1})
    
end
transition.to(objects_.dark,{time = 2000,alpha = 0,onComplete = functions.changeback_4})
end
game_.weather_1 = false
game_.weather_2 = false  
game_.weather_3 = false
game_.weather_4 = true
end
game_.killed = 0
...
if number_.mastermon == 0 and bol.masrun == true then
    bol.masrun = false   
    timer_.human_2 = timer.performWithDelay(5000, functions.helphuman_2, 3)
    bol.human_2 = true
    timer.cancel(timer_.master)
end
...
if number_.mobster <= (number_.holder*.90) and bol.bigrun == false then
    ...
    timer_.bolsstats = timer.performWithDelay(2000, functions.start, number_.bossing)    
    --functions.start()
    bol.bols = "false"
    bol.bolsstats = true 
    end
...
objects_._remover= display.newRect(0, 0, display.contentWidth*0.35, 10)
objects_._remover:setReferencePoint(display.CenterReferencePoint)
objects_._remover.x = w_ - 20
objects_._remover.y = display.contentHeight - 130;
external.physics.addBody(objects_._remover,"static",{density = 1, bounce = 0});
objects_._remover.myname = "end"
objects_._remover.alpha = 0
group[3]:insert(objects_._remover)

objects_.remover_= display.newRect(0, 0, display.contentWidth*0.25, 10)
objects_.remover_:setReferencePoint(display.CenterReferencePoint)
objects_.remover_.x = w_ - 240;
objects_.remover_.y = display.contentHeight - 160;
...
objects_.remover = display.newRect(0, 0, display.contentWidth*0.35, 10)
objects_.remover:setReferencePoint(display.CenterReferencePoint)
objects_.remover.x = w_ + 200;
objects_.remover.y = display.contentHeight - 160;
...
function functions.showobjects (event)
    
text_.level   = display.newEmbossedText("Level "..game_.leveltemp.." ".."Wave-"..game_.stagetemp, 10, 10, "BadaBoom BB", 32,{ 255, 255, 255, 255 });
text_.level:setReferencePoint(display.CenterLeftReferencePoint);
text_.level.x = w_ - 300;  
text_.level.y = 40;    
text_.level:setTextColor( 51, 51, 255 )
group[7]:insert(text_.level); 
...
group[7]:insert(text_.highscore);
...
elseif game_.stage >= 11  and game_.stage <= 15 or game_.stage >= 31  and game_.stage <= 35 then
transition.to(objects_.dark,{alpha = 1, time = 2000})
elseif game_.stage >= 16  and game_.stage <= 20 or game_.stage >= 36  and game_.stage <= 40 then
transition.to(objects_.dark,{alpha = 1, time = 2000})
...
objects_.barrel   = display.newImageRect("button/barrelbut/barrel.png", 68, 36)
...
group[7]:insert(objects_.barrel)
...
group[7]:insert(objects_.carpow)
...
--            else
            ...
            laser[number_.lasernumber] = display.newImageRect("button/laser/laser_1.png" ,display.contentWidth*0.09375,display.contentHeight*0.0625)
            ...
            group[7]:insert(laser[number_.lasernumber])
            --end  
        end
    end
    return true
...
local locbarrel = event.target
    ...
    barrel[number_.barrel] = display.newImageRect("button/barrelbut/barrel.png", 68, 36)
...
if event.object1.myname == "barrel" then
    ...
    number_.barrel = number_.barrel + 1
    barrelback[number_.barrel] = display.newImageRect("button/barrelbut/barreltap.png", 68, 36)
...
if ((event.object1.myname == "fence" and event.object2.myname == "runnerers") or 
    ...
    if event.object1.myname == "runnerers" or event.object1.myname == "masters" or event.object2.myname == "runnerers" or event.object2.myname == "masters" then
            game_.killed = game_.killed + 1
    end
...
if ((event.object1.myname == "sides" and event.object2.myname == "runnerers") or 
    ...
    elseif event.object2.myname == "runnerers" or event.object2.myname == "car" or event.object2.myname == "masters" or event.object2.myname == "bigmaster" then
        ...
        if event.object2.myname == "runnerers" or event.object2.myname == "masters" then
            game_.killed = game_.killed + 1
        end
        event.object2:removeSelf()
        event.object2.myname = nil    
...
local count_ = 0
text_.count   = display.newEmbossedText("", 10, 10, "BadaBoom BB", 80,{ 255, 255, 255, 255 });
text_.count:setReferencePoint(display.CenterReferencePoint);
text_.count.x = w_ ;  
text_.count.y = h_;    
...
function functions.taptutorial(event)
    if fing == true then
        ...
        local function endtutoria (event)
                    ...
                    timer.performWithDelay(500, function()
                        ...
                        elseif count_ == 3 then
                            timer.performWithDelay(300, functions.showobjects, 1)
                            ...
                            Runtime:addEventListener( "key", functions.pauseall )
                        end
                     count_ = count_ + 1   
                    end,4)
...
texttap.alpha = 1
 if tappings == false then
    monsters[number_.monster]:addEventListener("touch",functions.taptutorial) 
     tappings = true
 end
fing = false
...
Runtime:removeEventListener( "key", functions.pauseall )
